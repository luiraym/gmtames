...
set testsplit=maxmin

:: Set experiment name
set experiment=mtg_experiment_%testsplit%

:: Set task groupings to evaluate
:: ST groupings
set groupings="TA100"
...
python -m gmtames data --describeBaseDatasets --correlateBaseDatasets --testsplit %testsplit% --output %experiment%

:: Run MTG experiment
for %%g in (%groupings%) do (
	set grouping=%%g
	set grouping=!grouping:~1,-1%!
	python -m gmtames mtg --tasks !grouping! --testsplit %testsplit% --output %experiment% --device titanv
)
